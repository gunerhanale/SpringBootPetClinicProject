...
@Validated
@Service
@Transactional(rollbackFor = Exception.class) // Bu sayede check Exception'larda da rollback yapması gerektiğini belirtmiş olduk, şirketler böyle kullanırlar..
public class PetClinicServiceImpl implements PetClinicService {
	...
	@Override
	@CacheEvict(cacheNames = "allOwners", allEntries = true) // Cache edilen data nın güncellenmesi için ekledik
	public void createOwner(@Valid Owner owner) {
		ownerRepository.create(owner);

		SimpleMailMessage msg = new SimpleMailMessage();
		msg.setFrom("k@s");
		msg.setTo("gunerhanale@gmail.com");
...
//		if(true) throw new RuntimeException("testing rollback");   Rollback testi için bunu kullandık
	}
